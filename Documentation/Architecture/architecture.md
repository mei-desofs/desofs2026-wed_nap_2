# Architecture Analysis

This section consolidates the architecture artefacts used in Phase 1 analysis and design.

## Architecture Views

### Domain Model

- [Domain Model ](Diagrams/DomainModel/ArcadeHaven_Domain_Model.svg)
- [Domain Model (PlantUML)](Diagrams/DomainModel/domain_model.puml)

#### Level 1

- [Development View](Diagrams/Views/Level1/DevelopmentView.png)
- [Logical View](Diagrams/Views/Level1/LogicView.png)
- [Physical View](Diagrams/Views/Level1/PhysicalView.png)

#### Level 2

- [Development View](Diagrams/Views/Level2/DevelopmentView.png)
- [Logical View](Diagrams/Views/Level2/LogicView.png)
- [Physical View](Diagrams/Views/Level2/Physical.png)

#### Level 3

- [Logical View](Diagrams/Views/Level3/LogicView.png)

#### Use Case

- [Use Case](Diagrams/Views/UseCase/UseCase.png)

#### Source Files

- [Visual Paradigm Project](Diagrams/Views/DESOFS_Views.vpp)


## Dataflow

- [Dataflow Documentation](Dataflow/dataflow.md)
- [Dataflow Definition (JSON)](Dataflow/arcadehaven-dfd.json)
- [Dataflow Definition (PDF)](Dataflow/arcadehaven-dfd.pdf)
- [DFD Level 0 ](Dataflow/Level-0/DFD_Level0_Context.png)
- [DFD Level 1 - ArcadeHaven System ](Dataflow/Level-1/DFD_Level1_ArcadeHavenSystem.png)
- [DFD Level 1 - Auth API ](Dataflow/Level-1/DFD_Level1_AuthAPI.png)
- [DFD Level 1 - Database ](Dataflow/Level-1/DFD_Level1_Database.png)
- [DFD Level 1 - RAWG API ](Dataflow/Level-1/DFD_Level1_RAWGAPI.png)
- [DFD Level 2 - Game Management ](Dataflow/Level-2/DFD_Level2_GameManagement.png)

